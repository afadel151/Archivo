...
const FilteredFilesByModule = computed(() => {
    if (ModuleSearch.value != '') {
        return MyFiles.value.filter(file => file.module.module.toUpperCase().includes(ModuleSearch.value.toUpperCase()) || file.schoolyear.schoolyear.includes(ModuleSearch.value) || file.category.toLowerCase().includes(ModuleSearch.value.toLowerCase()))
    }
    ...
    <SuperAdmin>
        ...
        <Dialog>
            <DialogTrigger as-child>
                <Button variant="outline" class="absolute top-20 right-20">
                    Add A New File
                </Button>
            </DialogTrigger>
                ...
                <div class="grid gap-4 py-4">
                        ...
                        <Popover v-model:open="SchoolyearOpen">
                            ...
                            <PopoverContent class="col-span-3 p-0">
                                <Command>
                                    ...
                                    <CommandList>
                                        <CommandGroup>
                                            <CommandItem v-for="schoolyear in MySchoolYears" :key="schoolyear.id"
                                                :value="schoolyear.schoolyear" @select="(ev) => {
                    ...
                    <div class="grid gap-4 py-4">
                                        ...
                                        <CommandList>
                                            <CommandGroup>
                                                <CommandItem v-for="mod in MyModules" :key="mod.id"
                                                    ...
                                                    <span class="font-light text-red-400 ml-10">B{{ mod.battalion
                                                        }}</span>
                                                    <span class="font-light text-red-400 ml-10">{{ mod.sector }}</span>
                        ...
                        <div class="grid grid-cols-4 items-center gap-4">
                            ...
                            <Input id="name" v-model="FileName" name="file_name" placeholder="type a name for the file "
                                class="col-span-3" />
                        </div>
                        <div class="grid grid-cols-4 items-center gap-4">
        ...
        <div>
            <div class="pl-12 text-4xl mt-10 flex space-x-32">
                <span class="font-bold">My files</span>
                <!-- Search bar for modules -->
                <div class="relative w-full max-w-sm items-center">
                    <Input id="search" @input="HandleSearchInput"  type="text"
            ...
            <div class="grid grid-cols-6 gap-3 p-10">
                <div v-for="file in FilteredFilesByModule">
                    <Card>
                        <CardHeader>
                            <CardTitle>{{ file.name }} {{ file.category }}</CardTitle>
                        ...
                        <CardFooter class="flex justify-between px-6 pb-6">
                            <div class="flex flex-col">
                                <p class="text-xl">{{ file.module.module }} 
                                 <span
                                    class="text-lg text-blue-500">Semester {{
                                        file.module.semester }}</span> </p>
                            </div>
                            <Dialog>
                                <DialogTrigger as-child>
